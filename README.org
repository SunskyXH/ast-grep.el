...
- Optional, for live async search:
  ...
  - [[https://github.com/abo-abo/swiper][ivy + counsel]] when ~ivy-mode~ is active (consult and ivy do not mix; the backend is auto-selected)
...
- ~ast-grep-search-backend~ - Backend for ~ast-grep-search~ (default: ~auto~; also accepts ~consult~, ~ivy~, ~sync~). ~auto~ picks consult when available and ~ivy-mode~ is off, counsel/ivy when ~ivy-mode~ is on, and synchronous ~completing-read~ otherwise.
