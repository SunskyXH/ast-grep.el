...
| Backend selector | Runtime dependency | Selection behavior | Eask sandbox |
|------------------+--------------------+--------------------+--------------|
| ~auto~           | Optional consult or ivy/counsel | Uses ivy/counsel when ~ivy-mode~ is active and available; otherwise consult when available; otherwise sync | ~eask run script test:full~ |
| ~sync~           | None | Uses synchronous ~completing-read~ candidates | ~eask run script test:sync~ |
| ~consult~        | consult (and its dependencies) | Uses consult async search, falling back to sync if consult is unavailable | ~eask run script test:consult~ |
| ~ivy~            | ivy + counsel | Uses counsel/ivy async search, falling back to sync if ivy/counsel is unavailable | ~eask run script test:ivy~ |
